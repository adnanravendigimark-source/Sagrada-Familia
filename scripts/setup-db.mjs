...
async function createTables() {
  ...
  await sql`
    CREATE TABLE IF NOT EXISTS privacy_policy (
      ...
      empty_state_text TEXT NOT NULL DEFAULT 'This page hasn''t been filled in yet.',
      content JSONB NOT NULL DEFAULT '[]',
      no_index BOOLEAN NOT NULL DEFAULT false,
      no_follow BOOLEAN NOT NULL DEFAULT false,
      canonical_url TEXT NOT NULL DEFAULT '',
...
async function addHomepageCmsColumns() {
  ...
  // sections_json holds the "Why a Guided Tour", "Tower Access", "Practical
  // Info", and "Price Comparison" sections — grouped into one JSONB column
  // (rather than ~30 separate columns) since they're always saved together
  // from the same admin tab and read together on every homepage render.
...
// lib/posts.ts's rowToPost falls back to the original hardcoded CTA copy
// whenever these are empty, so running this never changes anything
// visible until an admin actually edits it). Also adds the table that
// backs "renaming a post's URL automatically redirects the old address".
...
async function addSeoColumns() {
  ...
  // column (e.g. "Yes — certified guide") — blank falls back to the
  // existing auto Yes/No derived from the tour's Type.
