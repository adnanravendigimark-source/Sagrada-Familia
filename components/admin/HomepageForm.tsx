    ...
    <form onSubmit={handleSubmit} className="space-y-5 pb-24">
        ...
        {TABS.map((tab) => (
          <button
            ...
            className={`flex items-center gap-1.5 rounded-xl px-3.5 py-2 text-sm font-medium transition ${
              activeTab === tab.key
                ? "bg-basilica-teal text-white shadow-sm"
      ...
      {activeTab === "content" && (
        <div className="space-y-5">
          ...
          <SectionCard title="“Why a Guided Tour” section" description="The timeline + what-you'll-learn section below the tour grid.">
          ...
          <SectionCard title="Tower Access section" description="Images live on the Images tab.">
            <div className="grid gap-5 sm:grid-cols-2">
          ...
          <SectionCard title="Price comparison" description="The table rows pull live from Tours & Tickets — this covers the heading above it and the column headers on the table itself.">
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              ...
              <Field label="Column 1" hint='e.g. "Live Guide" — set per tour via its "Price table: Live Guide" field, or leave blank to auto-fill from Type.'>
                <input value={content.sections.price.column1Label} onChange={(e) => updatePrice({ column1Label: e.target.value })} className={inputClass} />
              </Field>
              <Field label="Column 2" hint='e.g. "Tower Access" — set per tour via its "Price table feature" field.'>
                      ...
                      {tours.map((tour, i) => (
                        <tr key={tour.id} className={`border-t border-stone-200 ${i % 2 ? "bg-stone-50" : ""}`}>
                          ...
                          <td className="px-3 py-2 text-right">
                            <Link href={`/admin/tours/${tour.id}`} className="font-medium text-basilica-teal hover:underline">
                              Edit →
                            </Link>
                          </td>
                        </tr>
          ...
          <SectionCard title="Open Graph &amp; Twitter/X Preview" description="Used when the homepage is shared on Facebook, WhatsApp, iMessage, Twitter/X, etc. Leave blank to fall back to the hero's own title/description/image.">
            ...
            <div className="overflow-hidden rounded-xl border border-stone-200">
              ...
              <div className="bg-stone-50 p-3">
                <p className="truncate text-xs uppercase tracking-wide text-stone-400">sagrada-familia-tour.com</p>
      ...
      {activeTab === "images" && (
        <div className="space-y-5">
          ...
          <SectionCard title="Tower Access photos">
            <RepeatableList<GalleryImage>
        ...
        <div className="space-y-5">
          <SectionCard title="Focus keyword" description="The main phrase you want the homepage to rank for. Purely a writing aid — nothing here is sent to Google.">
            <Field label="Focus keyword">
              <input value={content.focusKeyword} onChange={(e) => update("focusKeyword", e.target.value)} className={inputClass} placeholder="e.g. Sagrada Familia guided tour" />
            </Field>
            {focusChecklist && (
          ...
          <SectionCard title="Brand colors" description="Changes these colors everywhere they're used across the whole site — buttons, links, hero background, ratings. Leave a field blank to keep the default.">
            <div className="grid gap-5 sm:grid-cols-2">
              <ColorField label="Primary (buttons)" value={content.theme.primary} fallback="#c4552f" onChange={(hex) => updateTheme({ primary: hex })} />
              <ColorField label="Secondary (links, accents)" value={content.theme.secondary} fallback="#0f5c5c" onChange={(hex) => updateTheme({ secondary: hex })} />
              <ColorField label="Dark (hero background)" value={content.theme.dark} fallback="#4a2545" onChange={(hex) => updateTheme({ dark: hex })} />
              <ColorField label="Accent (ratings, badges)" value={content.theme.accent} fallback="#d9a441" onChange={(hex) => updateTheme({ accent: hex })} />
            </div>
          </SectionCard>
