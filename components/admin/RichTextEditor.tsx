...
function cleanRichHtml(rawHtml: string, allowedHeadings: (1 | 2 | 3)[]): string {
  try {
    ...
    allElements.forEach((el) => {
      // Remove classnames from Word or ChatGPT
      el.removeAttribute("class");
      if (el.hasAttribute("style")) {
        const style = el.getAttribute("style") || "";
        // Only preserve table borders/alignments if present
...
export default function RichTextEditor({
  ...
  function handlePaste(e: React.ClipboardEvent<HTMLDivElement>) {
    ...
    if (pastedHtml && !isMarkdown) {
      // Clean HTML from web pages or rich text editors
  ...
  // Handle clicking inside the editor — detects clicks on images to open the image edit modal
  ...
  function handleImageModalSave({ url, alt, caption }: ImageModalData) {
    if (editingImageElementRef.current) {
      // EDIT MODE: Update the clicked image
    ...
    // INSERT MODE: Insert a new image
    if (!restoreSelection()) {
      setImageModalOpen(false);
      return;
    }
  ...
  const ToolbarButton = ({
    ...
    <button
      ...
      className={`rounded px-2.5 py-1 text-xs font-semibold transition ${
        active
          ? "bg-basilica-teal text-white shadow-sm ring-1 ring-basilica-teal"
    ...
    <div className="rounded-lg border border-stone-300 focus-within:border-basilica-teal focus-within:ring-1 focus-within:ring-basilica-teal">
      ...
      >
        ...
        <div className="flex items-center gap-1.5 px-2 py-0.5 text-xs text-stone-500">
          <span className="h-2 w-2 rounded-full bg-basilica-teal" />
          <span>Current:</span>
      ...
      <div
        ...
        className="rich-content max-w-none px-3 py-2.5 text-sm text-stone-900 outline-none [&_img]:cursor-pointer [&_img]:transition [&_img:hover]:ring-4 [&_img:hover]:ring-basilica-teal/40 [&_img:hover]:rounded-lg [&_figure]:cursor-pointer [&_figure:hover]:opacity-95"
        style={{ minHeight }}
      />
