...
export default function TourForm({
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        <div>
          ...
          <input
            ...
            placeholder="e.g. fast-track-guided-tour"
          />
        </div>
        <div>
      ...
      <div>
        <label className={labelClass}>Price table: "Live Guide" column (optional)</label>
        <input
          value={tour.priceTableColumn1 || ""}
          onChange={(e) => update("priceTableColumn1", e.target.value)}
          className={inputClass}
          placeholder="e.g. Yes — certified guide"
        />
        <p className="mt-1 text-xs text-stone-500">
          Shown in the homepage price-comparison table's "Live Guide" column for this tour. Leave blank to
          auto-fill Yes/No based on the Type above.
        </p>
      </div>

      <div>
        <label className={labelClass}>Price table: "Tower Access" column (optional)</label>
        <input
          value={tour.priceTableFeature || ""}
          onChange={(e) => update("priceTableFeature", e.target.value)}
          className={inputClass}
          placeholder="e.g. Passion or Nativity Tower"
        />
        <p className="mt-1 text-xs text-stone-500">
          Shown in the homepage price-comparison table's "Tower Access" column for this tour. Leave blank to show "No".
        </p>
      </div>
