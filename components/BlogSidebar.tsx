...
export default async function BlogSidebar({
  ...
  return (
    <aside className="space-y-8 lg:sticky lg:top-24 lg:self-start">
          ...
          <div className="p-5">
            ...
            <div className="mt-4 flex items-center justify-between border-t border-stone-900/10 pt-4">
              ...
              <a
                ...
                className="rounded-full bg-basilica-terracotta px-4 py-2 text-xs font-semibold text-white transition hover:bg-basilica-terracotta/90"
              >
                Book Now
              </a>
            </div>
          ...
          {related.map((post) => (
            <Link key={post.slug} href={`/blog/${post.slug}`} className="group flex gap-3">
              ...
              <div>
                ...
                <p className="mt-0.5 line-clamp-2 text-sm font-medium text-stone-900 group-hover:text-basilica-terracotta">
                  {post.title}
                </p>
              </div>
            </Link>
      ...
      <a
        href="/#tours"
        className="block rounded-2xl bg-basilica-teal/5 p-5 text-center text-sm font-semibold text-stone-900 transition hover:bg-basilica-teal/10"
      >
        Compare all tours &amp; tickets →
      </a>
    </aside>
  );
}
